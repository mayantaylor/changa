                          ...
                          const CkCallback &cb)
{
    ...
    Tipsy::TipsyReader r(session, bDoublePos, bDoubleVel);
    if (!r.status())
    {
        cerr << thisIndex << ": TreePiece: Fatal: Couldn't open tipsy file!" << endl;
        cb.send(0); // Fire off callback
                                ...
                                const CkCallback &cb)
{
    ...
    if (startParticle + myNumParticles > nTotalSPH + nTotalDark)
    {
        if (startParticle <= nTotalSPH + nTotalDark)
            myNumStar = startParticle + myNumParticles - (nTotalSPH + nTotalDark);
        else
    ...
    for (unsigned int i = 0; i < myNumParticles; ++i)
    {
        ...
        else if (i + startParticle < (unsigned int)tipsyHeader.nsph + tipsyHeader.ndark)
        {
            if (!bDoublePos)
        ...
        if (myParticles[i + 1].iOrder >= tipsyHeader.nsph)
            myParticles[i + 1].iOrder += tipsyHeader.nsph;
#endif
#if COSMO_STATS > 1
...
void TreePiece::readTipsyArray(OutputParams &params, const CkCallback &cb)
{
    ...
    xdr_u_int(&xdrs, &iDum);
    xdr_destroy(&xdrs);
    if (iDum == nTotalParticles)
    { // We've got a binary file; read it
        int64_t seek_pos;
        if (params.bVector)
    ...
    { // assume we've got an ASCII file
        int64_t nTot;
        ...
        for (int iDim = 0; iDim < nDim; iDim++)
        {
            ...
            for (int i = 0; i < myNumParticles; i++)
            {
                if (params.bFloat)
                {
                    ...
                    if (params.bVector)
                    {
                        Vector3D<double> vDummy = params.vValue(&myParticles[i + 1]);
                        vDummy[iDim] = dDummy;
...
#endif
#ifdef SPLITGAS
        if (myParticles[i + 1].iOrder < nTotalSPH)
            myParticles[i + 1].iSplitOrder() = i + nStartRead;
        if (myParticles[i + 1].iOrder >= nTotalSPH)
            myParticles[i + 1].iOrder += nTotalSPH;
#endif
        boundingBox.grow(myParticles[i + 1].position);
    }
                           ...
                           const CkCallback &cb)
{
    ...
    if (thisIndex + iOffset < (int)numTreePieces)
    { // Scan on
        if (verbosity > 1)
        {
    ...
    if (thisIndex < iOffset)
    { // No more messages are coming my way
                              ...
                              const int bCool)
{
    ...
    if (nSkip == 0)
        nSkip = 1;
    if (thisIndex % nSkip != iPass)
    { // N.B. this will only be the case
        // for the first pass.
        return;
    }
                            ...
                            const CkCallback &cb)
{
    ...
    for (int iPart = 1; iPart <= myNumParticles; iPart++)
    {
        ...
        {
            piSph[iGasOut] = (extraSPHData *)myParticles[iPart].extraData - mySPHParticles;
            iGasOut++;
        }
        if (myParticles[iPart].isStar())
        {
            piStar[iStarOut] = (extraStarData *)myParticles[iPart].extraData - myStarParticles;
            iStarOut++;
        }
    }
...
void TreePiece::oneNodeWrite(int iIndex,                  // Index of Treepiece
    ...
    if (myNumParticles > 0)
    {
        ...
        for (iPiece = 0; iPiece < numTreePieces; iPiece++)
        {
            ...
            if (nPartOut > 0)
            {
                ...
                ParticleShuffleMsg *shuffleMsg = new (saved_phase_len, saved_phase_len, nPartOut, nGasOut, nStarOut)
                     ...
                     pPart++, pPartOut++)
                {
                    *pPartOut = *pPart;
                    if (pPart->isGas())
                    {
                        shuffleMsg->pGas[iGasOut] = *(extraSPHData *)pPart->extraData;
                        iGasOut++;
                    }
                    if (pPart->isStar())
                    {
                        shuffleMsg->pStar[iStarOut] = *(extraStarData *)pPart->extraData;
                        iStarOut++;
                    }
                    ...
                    if (havePhaseData(PHASE_FEEDBACK) && (pPart->isGas() || pPart->isStar()))
...
void TreePiece::ioAcceptSortedParticles(ParticleShuffleMsg *shuffleMsg)
{
    ...
    if (myIOParticles == incomingParticlesArrived && incomingParticlesSelf)
    {
        ...
        if (verbosity > 1)
            ckout << thisIndex << " got ioParticles"
                  << endl;
        ...
        for (int iPart = 0; iPart < myNumParticles; iPart++)
        {
            ...
            {
                myParticles[iPart + 1].extraData = (extraSPHData *)&mySPHParticles[iGas];
                iGas++;
            }
            if (myParticles[iPart + 1].isStar())
            {
                myParticles[iPart + 1].extraData = (extraStarData *)&myStarParticles[iStar];
                iStar++;
            }
        }
        ...
        if (verbosity > 1)
            ckout << thisIndex << " contributing to ioAccept particles"
                  << endl;
...
void TreePiece::minmaxNCOut(OutputParams &params, const CkCallback &cb)
{
    ...
    for (unsigned int i = 1; i <= myNumParticles; ++i)
    {
        if ((TYPETest(&myParticles[i], params.iType) && TYPETest(&myParticles[i], params.iTypeWriting)) || params.iBinaryOut != 6)
        {
            if (params.bFloat)
            {
                if (params.bVector)
    ...
    for (unsigned int i = 1; i <= myNumParticles; ++i)
    {
        if ((TYPETest(&myParticles[i], params.iType) && TYPETest(&myParticles[i], params.iTypeWriting)) || params.iBinaryOut != 6)
        {
            ...
            else
            {
                ...
                { // tipsy binary array only does 32 bit ints
