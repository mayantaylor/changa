...
public:
  TreePieceStatistics(u_int64_t nol, u_int64_t nor, u_int64_t occ, u_int64_t nil, u_int64_t nir,
                      u_int64_t pil, u_int64_t pir, int na) : nodesOpenedLocal(nol), nodesOpenedRemote(nor), nodeInterLocal(nil),
...
public:
  ...
  extraSPHData *pGas;
  extraStarData *pStar;
  ParticleShuffleMsg(int nload, int npart, int nsph, int nstar) : nloads(nload), n(npart), nSPH(nsph), nStar(nstar) {}
};
...
inline double PoverRhoFloorJeans(double dResolveJeans, GravityParticle *p)
{
  ...
  if (l2 < e2)
    l2 = e2; /* Jeans scale can't be smaller than softening */
#endif
#endif
...
class Main : public CBase_Main
{
  ...
  public:
    ...
    {
      count = 0;
      tGrav = tuDot = tDD = tLoadB = tTBuild = tAdjust = tEmergAdjust = tKick = tDrift = tCache = 0.0;
    }
  };
...
struct NonLocalMomentsClient
{
  ...
  NonLocalMomentsClient() : clientTreePiece(NULL),
                            clientNode(NULL)
  {
  }

  NonLocalMomentsClient(TreePiece *tp, GenericTreeNode *node) : clientTreePiece(tp),
                                                                clientNode(node)
  {
  }
};
...
struct NonLocalMomentsClientList
{
  ...
  NonLocalMomentsClientList() : targetNode(NULL)
  {
  }

  NonLocalMomentsClientList(GenericTreeNode *node) : targetNode(node)
  {
  }
...
public:
  ...
  {
    if (verbosity > 1)
      ckout << "Deallocating treepiece " << thisIndex << endl;
    if (nStore > 0)
      delete[] myParticles;
    if (nStoreSPH > 0)
...
#endif
    if (verbosity > 1)
      ckout << "Finished deallocation of treepiece " << thisIndex << endl;
  }
  ...
  void savePhaseData(std::vector<double> &loads, std::vector<unsigned int> &parts_per_phase, double *shuffleloads, unsigned int *shuffleparts,
                     int shufflelen);
  void ResumeFromSync();
