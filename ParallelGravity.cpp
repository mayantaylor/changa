...
Main::Main(CkArgMsg *m)
{
    ...
    if (param.vPeriod.x == 0)
        param.vPeriod.x = 1.0e38;
    if (param.vPeriod.y == 0)
        param.vPeriod.y = 1.0e38;
    if (param.vPeriod.z == 0)
        param.vPeriod.z = 1.0e38;
    if (!param.bPeriodic)
    {
        param.nReplicas = 0;
        param.fPeriod = 1.0e38;
    ...
    {
        if (!param.bViscosityLimiter)
            param.iViscosityLimiter = 0;
    }
    ...
    {
        CkAssert(prmSpecified(prm, "dMsolUnit") && prmSpecified(prm, "dKpcUnit"));
    }
    ...
    if (param.bDoGas && !(param.bGasCooling || param.bGasAdiabatic || param.bGasIsothermal))
    {
...
#include "physconst.h"
    ...
    {
        param.dGasConst = param.dKpcUnit * KPCCM * KBOLTZ / MHYDR / GCGS / param.dMsolUnit / MSOLG;
        ...
        param.dThermalCondCoeffCode = param.dThermalCondCoeff * pow(1.5 * KBOLTZ / (0.6 * MHYDR), -3.5);
        /* Convert K2(T) to k2(u)  erg s^-1 (erg/gm)^-3/2 cm^-1 */
        param.dThermalCond2CoeffCode = param.dThermalCond2Coeff * pow(1.5 * KBOLTZ / (0.6 * MHYDR), -1.5);
        /* Convert to code units */
        param.dThermalCondCoeffCode /=
            pow(param.dErgPerGmUnit, -3.5) * param.dErgPerGmUnit * param.dGmPerCcUnit * pow(param.dKpcUnit * KPCCM, 2.0) / param.dSecUnit;
        param.dThermalCond2CoeffCode /=
            pow(param.dErgPerGmUnit, -1.5) * param.dErgPerGmUnit * param.dGmPerCcUnit * pow(param.dKpcUnit * KPCCM, 2.0) / param.dSecUnit;
        ...
        param.dEvapCoeffCode /=
            pow(param.dErgPerGmUnit, -2.5) * param.dGmPerCcUnit * pow(param.dKpcUnit * KPCCM, 2.0) / param.dSecUnit;
#else
    ...
    if (domainDecomposition == SFC_peano_dec)
        peanoKey = 3;
    if (domainDecomposition == SFC_peano_dec_2D)
        peanoKey = 2;
    if (domainDecomposition == SFC_peano_dec_3D)
        peanoKey = 3;
...
#undef xstr
    ...
    CProxy_Main(thishandle).setupICs(); // this method is threaded
}
...
void Main::getStartTime()
{
    ...
    if (param.csm->bComove)
    {
        ...
        if (prmSpecified(prm, "dRedTo"))
        {
            ...
            {
                if (param.nSteps != 0)
                    param.dDelta = (tTo - dTime) / (param.nSteps - param.iStartStep);
                else
                    /* set dDelta to a non-zero value so timestep
                     * adjustment works. */
                    param.dDelta = 1.0;
...
void Main::advanceBigStep(int iStep)
{
    ...
    while (currentStep < MAXSUBSTEPS)
    {
        ...
        if (!param.bStaticTest)
        {
            ...
            if ((param.bStarForm || param.bFeedback) && param.stfm->iStarFormRung > nextMaxRung)
            ...
            if (currentStep == 0)
                rungStats();
            if (verbosity)
                ckout << "MaxRung: " << nextMaxRung << endl;
         ...
         */
        if ((param.bStarForm || param.bFeedback) && param.stfm->isStarFormRung(activeRung))
        {
...
void Main::setupICs()
{
    ...
    if (param.iSIDMSelect != 0)
    {
        ...
        if (param.iSIDMSelect == 2)
        {                                                                                                                                // only for classical regime do this
...
void Main::initialForces()
{
    ...
    if (param.sinks.bDoSinksAtStart)
        doSinks(dTime, 0.0, 0);
...
void Main::doSimulation()
{
    ...
    for (int iStep = param.iStartStep + 1; iStep <= param.nSteps; iStep++)
    {
        ...
        if (verbosity)
            ckout << "Starting big step " << iStep << endl;
        startTime = CkWallTimer();
        CkPrintf("Elapsed time: %g\n", startTime - dSimStartTime);
        starCenterOfMass();
         ...
         */
        if ((param.bBenchmark == 0) && (bOutTime() || iStep == param.nSteps || iStop || iStep % param.iOutInterval == 0))
        {
            writeOutput(iStep);
        ...
        {
            if (param.iWallRunTime * 60. - (CkWallTimer() - dSimStartTime) < stepTime * 1.5)
            {
        ...
        if ((param.bBenchmark == 0) && (iStop || (param.iCheckInterval && iStep % param.iCheckInterval == 0)))
        {
...
///
void Main::writeTimings(int iStep)
{
...
void Main::calcEnergy(double dTime, double wallTime, const char *achLogFileName)
{
    CkReductionMsg *msg;
    ...
    else
    {
        ...
        if (param.csm->bComove)
        {
            dEcosmo += 0.5 * (a - csmTime2Exp(param.csm, dTimeOld)) * (dEnergy[2] + dUOld);
        }
        first = 0;
    }
...
void Main::writeOutput(int iStep)
{
    ...
    if (param.nSteps != 0 && param.bDoDensity)
    {
        ...
        if (param.bDoGas)
        { // recalculate gas densities for timestep
            if (verbosity)
...
int Main::adjust(int iKickRung)
{
    CkReductionMsg *msg;
    double a = csmTime2Exp(param.csm, dTime);
    double dDiffCoeff = (param.dMetalDiffusionCoeff > param.dThermalDiffusionCoeff ? param.dMetalDiffusionCoeff : param.dThermalDiffusionCoeff);
    double startTime = CkWallTimer();
...
{
    if (!param.bDtAdjust || iRung == 0)
        return;
    double startTime = CkWallTimer();

    if (verbosity)
        CkPrintf("Check for Emergency Adjust, Rung: %d\n", iRung);
...
void Main::updateSoft()
{
    ...
    if (param.bPhysicalSoft)
    {
        ...
        if (param.bSoftMaxMul && dFac > param.dSoftMax)
            dFac = param.dSoftMax;
...
{
    if (param.nGrowMass > 0 && dTime > param.dGrowStartT && dTime <= param.dGrowEndT)
    {
        double dDeltaM = param.dGrowDeltaM * dDelta / (param.dGrowEndT - param.dGrowStartT);
 ...
 */
int Main::DumpFrameInit(double dTime, double dStep, int bRestart)
{
    if (param.dDumpFrameStep > 0 || param.dDumpFrameTime > 0)
    {
        ...
        if (df[0]->dDumpFrameTime > 0)
        {
            ...
            while (df[0]->dTime + df[0]->dDumpFrameTime < dTime0 + param.dDelta * param.iStartStep)
            {
...
void Main::DumpFrame(double dTime, double dStep)
{
    ...
    for (i = 0; i < param.iDirector; i++)
    {
        ...
        else
        {
            ...
            void *Image = ((char *)msgDF->getData()) + sizeof(struct inDumpFrame);
